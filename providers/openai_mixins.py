"""Mixins for OpenAI-compatible provider - decoupling responsibilities.

This module contains focused mixins that handle specific aspects of the
OpenAI-compatible provider functionality:
...
class RequestBuilderMixin:
    ...
    including system prompts, tools, thinking mode, and provider-specific parameters.
    """
...
class ResponseConverterMixin:
    ...
    def convert_response(self, response_json: dict, original_request: Any) -> dict:
        ...
        if not content:
            # Some models require non-empty content.
...
class StreamProcessorMixin:
    """Mixin for processing streaming responses from OpenAI-compatible API.
