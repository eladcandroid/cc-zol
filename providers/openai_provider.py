...
class OpenAIProvider(
    ...
    def __init__(self, config: ProviderConfig):
        ...
        self._api_key = config.api_key or os.getenv("PROVIDER_API_KEY", "")
        self._base_url = (
            config.base_url
            or os.getenv("PROVIDER_BASE_URL", "https://api.openai.com/v1")
        ).rstrip("/")
    ...
    async def stream_response(
        ...
        if waited_reactively:
            ...
            logger.info("PROVIDER_STREAM: Reactive block detected, notified user")
        ...
        try:
            ...
            logger.error(f"PROVIDER_ERROR: {type(e).__name__}: {e}")
    ...
    async def complete(self, request: Any) -> dict:
        ...
        logger.info(
            f"PROVIDER_COMPLETE: model={body.get('model')} msgs={len(body.get('messages', []))} tools={len(body.get('tools', []))}"
        )
            ...
            if current_name == "Task":
                try:
                    ...
                    if args_json.get("run_in_background") is not False:
                        logger.info(
                            f"PROVIDER_INTERCEPT: Forcing run_in_background=False for Task {tc.get('id', 'unknown')}"
                        )
                        args_json["run_in_background"] = False
                        args = json.dumps(args_json)
                except Exception as e:
                    logger.warning(
                        f"PROVIDER_INTERCEPT: Failed to parse/modify Task args: {e}"
                    )
