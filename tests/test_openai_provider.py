...
async def test_stream_response_text(openai_provider):
    ...
    with patch.object(
        ...
        events = []
        async for event in openai_provider.stream_response(req):
            events.append(event)
...
@pytest.mark.asyncio
async def test_stream_response_thinking_reasoning_content(openai_provider):
...
@pytest.mark.asyncio
async def test_tool_call_stream(openai_provider):
